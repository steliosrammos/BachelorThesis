...
# Experiments Ionosphere Dataset
...
# Select dataset
data = pd.read_csv(base_path+"data/external/breast_cancer.csv", sep=",")
data.head()
...
```{python}
test_data.to_csv(base_path+"data/external/test_ionosphere.csv", sep=";", index=False)
train_data.to_csv(base_path+"data/external/train_ionosphere.csv", sep=";", index=False)
```

# Unbiased Model

```{python}
train_data = pd.read_csv(base_path+"data/external/train_ionosphere.csv", sep=";")
...
```{python}
biased_data.loc[(biased_data.a34 == 0) | (biased_data.a09 < 0.5), 'got_go'] = 0
biased_data.loc[(biased_data.a34 == 0) | (biased_data.a09 < 0.5), 'class'] = np.nan
biased_data.loc[biased_data.a34 == 0, :].head()
```
...
```{python}
biased_data.to_csv(base_path+"data/external/biased_train_ionosphere.csv", sep=";", index=False)
```
