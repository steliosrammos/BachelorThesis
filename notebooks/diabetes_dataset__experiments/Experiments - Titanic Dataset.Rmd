...
```{python}
biased_data.loc[(biased_data['MDVP:Fo(Hz)'] < 120) | (biased_data['MDVP:Flo(Hz)'] > 130), 'got_go'] = 0
biased_data.loc[(biased_data['MDVP:Fo(Hz)'] < 120) | (biased_data['MDVP:Flo(Hz)'] > 130), 'class'] = np.nan
```
...
```{python}
X_lbld = labeled.iloc[:, :-2]
y_lbld = labeled.iloc[:, -1]

roc_scores = []
pr_scores = []
brier_scores = []

for train_index, test_index in skf.split(X_lbld, y_lbld):
    
    X_train = X_lbld.iloc[train_index]
    y_train = y_lbld.iloc[train_index]
...
print("ROC: {0:9.4f} \n".format(np.array(roc_scores).mean()))
```
