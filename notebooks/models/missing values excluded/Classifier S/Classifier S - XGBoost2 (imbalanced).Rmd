...
In a first attempt to correct the imbalance I added more instances of class 0 that were available from a later date.

In this notebook we will experiment with different splitting approaches for our test/valid sets and see how that affects the classifier's performance. Those approaches are namely: 
...
ISOTONIC
1. KFold: <br>
    -> ROC AUC (avrg): 0.7595<br>
    -> PR AUC (avrg): 0.8839<br>
    -> Brier Loss: 0.1533<br>
2. Stratified KFold:<br>
    -> ROC AUC (avrg): 0.7595 <br>
    -> PR AUC (avrg): 0.8821<br>
    -> Brier Loss: 0.1524<br>
    
SIGMOID
1. KFold: <br>
    -> ROC AUC (avrg): 0.7625<br>
    -> PR AUC (avrg): 0.8852<br>
    -> Brier Loss: 0.1541<br>
2. Stratified KFold:<br>
    -> ROC AUC (avrg): 0.7632<br>
    -> PR AUC (avrg): 0.8850<br>
    -> Brier Loss: 0.1530<br>
...
```{python}
base_path = '/Users/steliosrammos/Documents/Education/Maastricht/DKE-Year3/BachelorThesis/bachelor_thesis/'
data = pd.read_csv(base_path+"data/interim/data_extra_nogo_2018.csv", sep=";")

# Add a weight column for the bias correction coefficients
data.insert(data.shape[1]-2, "weight", pd.Series())

data_s = data.drop(['uuid', 'finished_treatment'],axis=1)
data_s = data_s.fillna(data.mean())
...
def get_best_xgb_estimator(X_train, X_valid, y_train, y_valid, grid_search=False, verbose=False):
    ...
    if grid_search == True: 
        ...
        xgbest = XGBClassifier(n_jobs=6, reg_lambda=0.8, scale_pos_weight=0.2, learing_rate=0.05, max_depth=30, n_estimators=150, objective='binary:logistic')
...
for train_index, valid_index in sss.split(X, y):
    ...
    for index in X_valid.index:
        data.loc[index, "weight"] = prob_s_pos / predicted_prob_s[index]
...
def plot_calibration_curve(y_true, y_predicted):
    ...
    ax2.hist(y_predicted, range=(0, 1), bins=10, histtype="step", lw=2)
...
# prob_pos = np.concatenate(predicted_prob_s,axis=0)
