...
train, valid = data_h2o.split_frame(ratios=[0.75], seed=-1)
```
...
corr_coeff_lab = corr_coeff[~y.isna()].values

X_unlab = X.loc[y.isna(), :].values

sets =  train_test_split(X_lab, y_lab, corr_coeff_lab, test_size=0.33, random_state=1, stratify = y_lab)
X_train, X_test, y_train, y_test, corr_coeff_train, corr_coeff_test = sets
```
